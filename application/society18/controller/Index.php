<?php
namespace app\society18\controller;
use app\common\controller\Frontend;
use app\accsmarket\model\Order;
...
class Index extends Frontend
{
    ...
    public function search()
    {
        if (input("email")) {
            ...
            $orders=db("order")->where($where)->order("createtime desc")->paginate(5);
    ...
    {
        if ($category_id) {
          $categorys=db('category')->where('type','goods')->where('id',$category_id)->order("weigh desc")->select();
        }else{
          $categorys=db('category')->where('type','goods')->order("weigh desc")->select();
        }
    ...
    {   
        if ($id) {
            ...
            if ($goods['category_id']) {
                $goods['category_image']=db("category")->where('id',$goods['category_id'])->value('image');
    ...
    public function pay($goods_id,$category_id,$name,$goods_price,$goods_num,$email,$type='alipay')
    {     
