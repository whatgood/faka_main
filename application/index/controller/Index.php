...
class Index extends Frontend
{
    ...
    protected $layout = '';
    function __construct(\app\index\model\Category $category,\app\index\model\Goods $goods)
    {
        parent::__construct();
    ...
    {
        return $this->view->fetch('index',['categories'=>$this->category->categories($id)]);
    }
    public function trade($id)
    {
        if ($id) {
            $goods=$this->goods->goods($id);
            if ($goods) {
                return $this->view->fetch('trade',['goods'=>$goods]);
            }else{
                $this->success("请先购买商品", '/');
            }
        } else {
